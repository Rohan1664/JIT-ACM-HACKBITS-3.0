...
const EVENT_TYPE_COLORS = {
  ...
  discussion: 'from-sky-0 to-blue-0',
  announcement: 'from-sky-0 to-blue-0',
  mentorship: 'from-sky-0 to-blue-0'
};

// Static day selector data
...
// Memoized shooting star positions
const SHOOTING_STAR_POSITIONS = Array.from({ length: 3 }, (_, i) => ({
  top: `${Math.random() * 60}%`,
  left: `${Math.random() * 100}%`,
...
const EventFlow = () => {
  ...
  const handleEventClick = useCallback((index) => {
    setSelectedEvent(prev => prev === index ? null : index);
  }, []);
  ...
  // Memoize getEventTypeColor function
  const getEventTypeColor = useCallback((type) => {
    return EVENT_TYPE_COLORS[type] || 'from-sky-0 to-blue-0';
  }, []);
    ...
    <LazyMotion features={domAnimation}>
      <section id="eventflow" className="relative py-16 md:py-20 lg:py-28 overflow-hidden bg-gradient-to-b from-[#020617] via-[#030b1a] to-[#000000]">
        ...
        <div className="absolute inset-0">
          {/* Star Field - Using pre-generated positions */}
          <div className="absolute inset-0">
          ...
          {/* Shooting Stars - Using pre-generated positions */}
          <div className="absolute inset-0 pointer-events-none">
            {SHOOTING_STAR_POSITIONS.map((star, i) => (
              <motion.div
                key={`shooting-${i}`}
