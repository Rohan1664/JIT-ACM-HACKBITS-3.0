...
// Note: You'll need to create these image files or replace with your actual images
...
const TeamPage = () => {
  ...
  const executiveTotalPages = Math.ceil(executiveMembers.length / executivePerPage);
  const acmTotalPages = Math.ceil(acmMembers.length / acmPerPage);

  const currentExecutiveMembers = executiveMembers.slice(
    (executivePage - 1) * executivePerPage,
    executivePage * executivePerPage
  );

  const currentAcmMembers = acmMembers.slice(
    (acmPage - 1) * acmPerPage,
    acmPage * acmPerPage
  );

  // Pagination handlers
  const nextExecutivePage = () => {
    if (executivePage < executiveTotalPages) {
      setDirection(1);
      setExecutivePage(executivePage + 1);
    }
  };

  const prevExecutivePage = () => {
    if (executivePage > 1) {
      setDirection(-1);
      setExecutivePage(executivePage - 1);
    }
  ...
  const handleAcmPageChange = (page) => {
    if (page > acmPage) {
      setDirection(1);
  ...
  return (
    ...
    >
      ...
      <div className="container relative z-10 px-4 mx-auto sm:px-6 lg:px-8">
        ...
        {/* Sponsors Section - Continuous Slideshow */}
        <motion.div
          ref={ref}
          initial="hidden"
        ...
        {/* Executive Members Section with Pagination */}
        <motion.div
          variants={containerVariants}
          initial="hidden"
          animate={inView ? 'visible' : 'hidden'}
